...
public async Task<IActionResult> ChangePassword(string oldPassword, string newPassword, string RePassword)
{
    ...
    return RedirectToAction("Profile", "Staff", new { id = accountId });
        ...
        public IActionResult AppoitmentList(string status = "all", string search_doctor = "", string sortColumn = "AppointmentId", string sortDirection = "asc", int pageNumber = 1)
        {
            ...
            {
                appointmentsQuery = appointmentsQuery.Where(o => o.Option.Status == status);
            }
            // Apply sorting
            switch (sortColumn)
            {
	  ...
	  .OrderByDescending(o => o.Option.DateWork) // Sắp xếp theo AppointmentDate giảm dần
	  ...
	  .ToList();
        ...
        public IActionResult AppointmentDetail(string id)
        {
            ...
            var appointment = dc.Orders
                ...
                .Select(o => new AppointmentDetailViewModel
                {
                    ...
                    DoctorGender = o.Option.DidNavigation.Gender ?? "Male",
        ...
        //-------------------------------------------------------------------------------------------------------------
        public IActionResult ServiceAppointList(string status = "all", string search_service = "", string sortColumn = "AppointmentId", string sortDirection = "asc", int pageNumber = 1)
        {
            int pageSize = 10;
