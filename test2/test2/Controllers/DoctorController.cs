...
namespace test2.Controllers
{
    ...
    public class DoctorController : Controller
    {
        ...
        public IActionResult ViewAppointment(string id)
        {
            ...
            // Log giá trị oid
            _logger.LogInformation("OID nhận được trong DoctorProfile: {Oid}", id);

            // Kiểm tra xem ID của người dùng có khớp với ID trong URL không
            if (userId != id)
            {
                _logger.LogWarning("Người dùng cố gắng truy cập hồ sơ không thuộc về họ: {UserId} đã cố gắng truy cập {TargetId}", userId, id);
                return Forbid("Bạn không có quyền truy cập vào hồ sơ này."); // Ngăn chặn truy cập nếu ID không khớp
            }
        ...
        public IActionResult ViewAppointmentDetail(string appointmentDetail)
        {
            ...
            _logger.LogInformation(appointmentDetailViewModel.Status);
